...
def metric_card(title, value, description, card_class="metric-card-neutral",
...
def make_error_timeseries(errors_h, errors_v, dop_list):
    ...
    axes[0].axhline(np.nanmean(errors_h), color="#1D9E75", linestyle="--",
                    linewidth=0.8, label=f"Mean = {np.nanmean(errors_h):.1f} m")
    axes[0].set_ylabel("Horizontal error (m)")
    axes[0].legend(fontsize=8)
    axes[0].grid(True, alpha=0.3)
    axes[1].plot(epochs_min, errors_v, color="#D85A30", linewidth=0.9)
    axes[1].axhline(np.nanmean(errors_v), color="#993C1D", linestyle="--",
                    linewidth=0.8, label=f"Mean = {np.nanmean(errors_v):.1f} m")
    ...
    axes[2].axhline(2.0, color="#854F0B", linestyle="--",
                    linewidth=0.8, alpha=0.6, label="HDOP = 2.0 threshold")
...
with st.sidebar:
    ...
    max_epochs = st.slider(
        ...
        help=(
            "10 = 5 min · 60 = 30 min · 120 = 1 hour · 240 = 2 hours. "
...
if not run_button:
    ...
    with st.spinner("Processing GPS data — this takes 1–3 minutes..."):
        try:
    ...
    with col_snr:
        if results["snr_results"]:
            ...
            if not advanced_mode:
                st.caption(
                    "Green = strong signal, red = weak. White gaps = satellite below horizon. "
                    "Row label color: green = clean, amber = suspect."
                )
